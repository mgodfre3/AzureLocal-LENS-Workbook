...
const capacityQuery = [
    ...
    "// Join VM vCPU and memory totals per cluster RG (uses resources table, same pattern as Azure Local VMs tab)",
    "| join kind=leftouter (",
    "    resources",
    "    | where type == \"microsoft.hybridcompute/machines\"",
    "    | where kind == \"HCI\"",
    "    | extend vCPUs = toint(properties.detectedProperties.logicalCoreCount)",
    "    | extend memGB = toint(properties.detectedProperties.totalPhysicalMemoryInGigabytes)",
    "    | summarize vmVCPUTotal = sum(vCPUs), vmMemoryTotalGB = sum(memGB) by vmRG = tolower(resourceGroup)",
    ") on $left.hciClusterRG == $right.vmRG",
    "// Join AKS Arc vCPU totals per cluster RG (uses resources table, same pattern as AKS Arc Clusters tab)",
    "| join kind=leftouter (",
    "    resources",
    "    | where type == \"microsoft.kubernetes/connectedclusters\"",
    "    | where tostring(properties.infrastructure) == \"azure_stack_hci\"",
    "    | extend totalCoreCount = toint(properties.totalCoreCount)",
    "    | summarize aksVCPUTotal = sum(totalCoreCount) by aksRG = tolower(resourceGroup)",
